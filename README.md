...
- [Dashboard (Luminosidade)](#-dashboard-luminosidade)
...
## 📊 Dashboard (Luminosidade)

Além da PWA, o projeto inclui um **dashboard** para visualizar a luminosidade salva no histórico (FIWARE / STH‑Comet), gerando um HTML com gráficos (Chart.js).

Arquivos em `Dashboard/`:
- `Dashboard/dashboard_luminosidade.py` — versão direta (gera e abre `dashboard_luminosidade.html`).
- `Dashboard/luminosity_dashboard.py` — versão mais completa (3 gráficos: todo período / 1 min / 5 min; média e linha de anotação).
- `Dashboard/dashboard_luminosidade.html` — exemplo de saída gerada.
...
### Configuração (API do STH‑Comet)

No `Dashboard/dashboard_luminosidade.py`, ajuste o dicionário `CONFIG` conforme seu ambiente:
- `base_url`: URL do STH‑Comet (ex.: `http://SEU_IP:8666`)
- `entity_type` / `entity_id`: entidade da lâmpada (ex.: `urn:ngsi-ld:Lamp:001`)
- `attribute`: atributo (ex.: `luminosity`)
- `service` e `service_path`: headers FIWARE obrigatórios

Importante: o STH‑Comet normalmente retorna **400** se os headers `fiware-service` e `fiware-servicepath` não forem enviados.

### Como executar

Rode o script a partir da pasta `Dashboard/` para gerar/atualizar o HTML ali mesmo:

```bash
cd Dashboard
python dashboard_luminosidade.py
```

O script:
- consulta a API (últimos `lastN` registros)
- filtra janelas (1 min e 5 min)
- escreve o arquivo `dashboard_luminosidade.html`
...
<details>
<summary>📊 Dashboard retorna erro 400/404 na API</summary>

- Confirme se o `base_url` aponta para o STH‑Comet (porta e caminho corretos).
- Verifique os headers FIWARE (`fiware-service` e `fiware-servicepath`).
- Confira se a entidade e atributo existem (`entity_id`, `entity_type`, `attribute`).
</details>

<details>
<summary>📊 Dashboard abre, mas está “sem dados”</summary>

- O histórico só aparece se sua stack estiver gravando valores no STH‑Comet.
- Valide no console do firmware se a luminosidade está sendo publicada e ingerida.
- Aumente `lastN` no `CONFIG` para buscar mais amostras.
</details>
